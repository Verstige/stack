            ...
            <div class="flex flex-col gap-2">
                <div class="flex items-end gap-2">
                    <x-forms.input required id="update_check_frequency" label="Update Check Frequency"
                        placeholder="0 * * * *"
                        helper="Frequency (cron expression) to check for new Stack versions and pull new Service Templates from CDN.<br>You can use every_minute, hourly, daily, weekly, monthly, yearly.<br><br>Default is every hour." />
                    ...
                    @if (!is_null(config('constants.stack.autoupdate', null)))
                        <div class="text-right">
                            <x-forms.checkbox instantSave
                                ...
                                checked="{{ config('constants.stack.autoupdate') }}" label="Enabled" />
                        </div>
                    @else
                        <x-forms.checkbox instantSave id="is_auto_update_enabled" label="Enabled" />
                    @endif
                </div>
                @if (is_null(config('constants.stack.autoupdate', null)) && $is_auto_update_enabled)
